...
attributes per-model spend/tokens to the cloud quota rows Jarvis budgets.

Spec: master_summary §12.4 ("quota.py — LiteLLM spend & token tracking";
LiteLLM logging Path A).

── Phase A (current) ─────────────────────────────────────────────────────────
Code-only. The live DB is NOT yet wired (Path A deferred — see
PHASE_B_litellm_path_a.md, target instance monarch-postgres:5433). When
...
from ..schema import QuotaStatus
...
# spend_logs.model value → CloudQuota key. Only cloud providers Jarvis budgets.
# Local models (qwen3.6-*, phi4-mini, …) are $0 and intentionally unmapped.
# claude_pro_* and anthropic_api_direct never traverse LiteLLM (Pro descoped;
# direct is a forward-compat hook) — they stay dormant by design.
_MODEL_TO_QUOTA: Dict[str, str] = {
    "deepseek-v4-flash":          "deepseek_v4_flash",
    "deepseek/deepseek-v4-flash": "deepseek_v4_flash",
    "kimi-k2.6":                  "kimi_k2_6",
    "kimi-k2-6":                  "kimi_k2_6",
    "moonshot/kimi-k2-6":         "kimi_k2_6",
}
