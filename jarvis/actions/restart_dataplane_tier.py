...
tier (T3 content/batch, T5 small-helper) by shelling out to the operator's
idempotent per-tier launch script (~/bin/t3-up | ~/bin/t5-up). Those scripts
own the launch flags/ports and recover a single tier WITHOUT bouncing the whole
dataplane — `inference-up` kills the entire `inference` session, so it is NOT a
single-tier restart mechanism (verified against disk during P3.1 execution).

Scope is locked to T3/T5 because they are:
...
Excluded by design: T1 (Hard Constraint #4 → always Tier 3), T4 (GPU), T2/T6
...
class RestartCpuDataplaneTier(Action):
    ...
    description = "Restart a crashed zero-VRAM CPU dataplane tier (T3/T5) via idempotent per-tier launch script"
