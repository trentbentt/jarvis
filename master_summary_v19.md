...
| AUTHORITY_SPEC_v19 | ✅ All 8 items + Quota Cascade Policy ratified | **[v19 update]** Decision 5 walkthrough across 2026-05-21 → 2026-05-24 closed. Items 1-5 banked in commits `50692bd` / `414d5b2` / `f0675da` (Tier 1/2/3 action lists, Hard Constraints, Overnight Workload Window 23:00-07:00 ET, Substrate Pressure Cascade continuous-intensity-band, bypass severity ladder). Items 6-8 + Quota Cascade Policy closed 2026-05-24: Item 6 Pro tier estimation **descoped** (Pro is workflow-tier-zero, not Jarvis-routed); Item 7 promotion threshold **N=12** uniform across both rungs (changed from N=10 draft during ratification); Item 8 **strict cold-start** (no override at introduction; material behavior change re-enters at Tier 3); Quota Cascade Policy **20% / 10% with fullest-peer rotation** between DeepSeek V4 Flash ↔ Kimi K2.6 + drain phase per-percent notification overlay. |
...
- **Items 6-8 + Quota Cascade Policy closed 2026-05-24.** Item 6 — Pro tier estimation **descoped**: Pro is workflow-tier-zero (operator-driven, not Jarvis-routed); Pro auth via Claude Code subscription path, not LiteLLM, so Pro requests are opaque to `spend_logs` regardless. Re-open condition: automated Pro-1 → Pro-2 → T6 failover built. Item 7 — promotion threshold **N=12 uniform across both rungs** (Tier 3 → Tier 2 → Tier 1), total cold-start to silent operation minimum 24 runs; changed from N=10 draft during ratification. Item 8 — **strict cold-start ratified**: no override at introduction; material behavior change to existing action re-enters at Tier 3. Quota Cascade Policy — **20% / 10% with fullest-peer rotation** between DeepSeek V4 Flash ↔ Kimi K2.6 (Tier 2 routing, logged events); drain phase engages when both peers < 10% with per-percent notification overlay (new authority primitive: Tier 2 routing + operator-facing notification, scoped to Quota Cascade Policy, not promoted to general AUTHORITY_SPEC primitive).
...
- **Open audit items carried.** A3 (CLAUDE.md materially stale — Tier D backlog), A5 (state.json schema_version "0.1.0" — small mission), A7 (v18 Hermes brainstorm document may not exist as a discrete artifact — Decision 2 prerequisite question), A12 (SystemModel 9-domains vs 8 — cosmetic), A13 (HANDOFF intra-doc drift — Tier D). Plus B8/B10 (7-doc rubric as discipline, deserves its own ratification), B13 (session_start/checkpoint/wrap rituals). Closed 2026-05-24: A6 (commit `b524bb0` — `deepseek_v3 → deepseek_v4_flash` rename in state.py); A10 (Item 7 ratification changed value to N=12, not just confirming the prior N=10).
...
**Decision 4 — Cloud routing chain (CLOSED 2026-05-19; amended 2026-05-24).**
*Statement (original 2026-05-19):* Pro (×2) → DeepSeek V4 Flash → Kimi K2.6 → Haiku 4.5 → Anthropic API direct. Cowork retired.
*Amendment v1 (2026-05-24) — structural class reframe:* Three classes — **workflow-tier-zero** (Pro, operator-driven; not Jarvis-routed), **peer rotation** (DeepSeek V4 Flash ↔ Kimi K2.6, fullest-peer rotation per AUTHORITY_SPEC §Quota Cascade Policy), **emergency rung** (Anthropic API direct, Tier 3 per-call invocation; vestigial — doctrine-forward, not yet wired). Original priority chain preserved as tie-breaker when task class is ambiguous.
*Amendment v2 (2026-05-24) — Haiku 4.5 deprecated:* Pricing parity with DeepSeek V4 Flash at lower capability makes Haiku redundant. Removed from cascade. Re-open condition: a future provider with a genuinely-distinct latency profile re-justifies a latency niche class.
*Open small missions related to Decision 4:* Throughput-tier model ambiguity (used in leads and financial opencode.jsonc but not explicitly placed in cascade); no Moonshot/Kimi key (peer-rotation rung is half-wired until key acquired). Two-Pro saturation handling is no longer a Decision 4 open mission — Pro descoped from Jarvis routing per Decision 5 Item 6 closure 2026-05-24.

**Decision 5 — Jarvis authority model (CLOSED 2026-05-24 — all 8 items + Quota Cascade Policy ratified).** See What-Changed-v19 bullets above for the per-Item walkthrough state and commit refs. Item 6 Pro tier estimation descoped (Pro is workflow-tier-zero); Item 7 N=12 uniform; Item 8 strict cold-start; Quota Cascade Policy 20% / 10% fullest-peer rotation with drain-phase notification overlay.
...
| A2 | AUTHORITY_SPEC Items 6-8 + Quota Cascade thresholds | ✅ Closed 2026-05-24 | Items 6/7/8 + Quota Cascade Policy ratified; AUTHORITY_SPEC_v19.md is doctrine-complete (see Tier B-D5 outcomes below). | — |
| A3 | Decision 4 cascade composition formalization | ⏳ Small mission | Throughput-tier model ambiguity used in leads/financial `opencode.jsonc` but not explicitly placed in cascade; Moonshot/Kimi key not acquired (peer-rotation rung half-wired). Two-Pro saturation handling closed 2026-05-24 — Pro descoped from Jarvis routing per Decision 5 Item 6. | 30 min docs + ongoing |
...
| B-D5.6 | Decision 5 Item 6 — Pro tier estimation | ✅ Descoped 2026-05-24 | Pro is workflow-tier-zero (operator-driven, not Jarvis-routed); Pro auth via Claude Code subscription path, not LiteLLM. Re-open condition: automated Pro-1 → Pro-2 → T6 failover built. |
| B-D5.7 | Decision 5 Item 7 — Promotion threshold N | ✅ Ratified 2026-05-24 | **N=12** uniform across both rungs (Tier 3 → Tier 2 → Tier 1). Total cold-start to silent operation: minimum 24 operator-acknowledged successful runs. Changed from N=10 draft value during ratification. |
| B-D5.8 | Decision 5 Item 8 — Cold-start rule | ✅ Ratified 2026-05-24 | Strict cold-start — all new actions begin Tier 3, **no override at introduction**. Material behavior change to existing action re-enters at Tier 3 (changed action treated as new for cold-start purposes). |
| B-D5.Q | Decision 5 Quota Cascade thresholds | ✅ Ratified 2026-05-24 | **20% / 10% with fullest-peer rotation** between DeepSeek V4 Flash ↔ Kimi K2.6. Drain phase engages when both peers < 10% — per-percent notification overlay (new authority primitive: Tier 2 routing + operator-facing notification, scoped to Quota Cascade Policy). |
...
| C3 | `quota.py` listener | ⏳ Spec-only | C2 closed 2026-05-24 (Path A ratified). State.py provider rows: `deepseek_v3 → deepseek_v4_flash` rename landed in `b524bb0`. **No `haiku_4_5` row** (deprecated per Decision 4 v2). **`anthropic_api_direct` row deferred** to operational need. | 3-4 hr. Implements the Quota Cascade Policy (AUTHORITY_SPEC §Quota Cascade Policy) against prepaid balances of the peer-rotation rung (DeepSeek V4 Flash ↔ Kimi K2.6) and emergency rung (Anthropic API direct, when wired). Pro tracking descoped 2026-05-24 — Pro is workflow-tier-zero, opaque to LiteLLM, not in scope. |
